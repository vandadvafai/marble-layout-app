...
This is the engine's first and only generator. The `balanced` strategy is
a thin wrapper around it. The four other strategies will eventually share
this same generator with different ordering, scoring, and randomisation
hooks — keeping the geometric work in one place.
...
Limitations (intentional for MVP):
  ...
  - Slabs taller than the row are cropped (slab_polygon reflects only
    the used portion). Offcut tracking will land later.
...
class RowBasedStrategy(PlacementStrategy):
    ...
    def generate(self, ctx: StrategyContext) -> StrategyResult:
        ...
        while slab_idx < len(slabs) and cursor_y < maxy:
            ...
            if new_pieces:
                # Successful placement: commit pieces and consume the slab
                # from inventory.
                pieces.extend(new_pieces)
                slab_idx += 1
            else:
                # No valid geometry at this cursor position. Record where
                # the slab would have gone, advance the cursor, but keep
                # the slab in inventory so the next position gets a chance.
                markers.append(
                    ReviewMarker(
            ...
            # Cursor always advances. This is what guarantees the loop
            # terminates even when the same slab keeps failing.
            cursor_x += slab.width
            if cursor_x >= maxx:
                cursor_y += row_height
                cursor_x = minx
